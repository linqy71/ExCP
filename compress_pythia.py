...
# 当前剪枝策略
def my_prune(exp_tensor, exp_sq_tensor, tensor, j = 1):
    # res_exp = torch.where((exp_tensor.abs()) > j * (exp_tensor.abs()).mean(), exp_tensor, 0)
    # res_exp = torch.where(tensor == 0, 0, res_exp)
    # res_exp_sq = torch.where((exp_tensor.abs()) > j * (exp_tensor.abs()).mean(), exp_sq_tensor, 0)
    # res_exp_sq = torch.where(tensor == 0, 0, res_exp_sq)
    ...
    for i, line in enumerate(exp_tensor):
        if line.abs().mean() < prune_value:
            exp_tensor[i] = prune_line
            exp_sq_tensor[i] = prune_line
            tensor[i] = prune_line

    return tensor, (tensor == 0).sum(), exp_tensor, exp_sq_tensor, (exp_tensor == 0).sum()
    ...
    with safe_open(os.path.join(args.ref_checkpoint_path, "model.safetensors"),\
        ...
        with safe_open(os.path.join(args.checkpoint_path, "model.safetensors"),\
            ...
            for ind, k in enumerate(ref_weights.keys()):
                ...
                # 当前剪枝策略
                residual_tensor, remove, pruned_opt_v, pruned_opt_m, remove_opt = my_prune(opt_v, opt_m, residual_tensor, args.prune_beta)
                ...
                # 去量化
                saved_checkpoint[k] = {}
                saved_checkpoint[k]['weights_i'] = torch.Tensor(residual_tensor)
                saved_checkpoint[k]['opt_v_i'] = torch.Tensor(pruned_opt_v)
                saved_checkpoint[k]['opt_m_i'] = torch.Tensor(pruned_opt_m)
